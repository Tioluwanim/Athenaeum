...
class User(Base):
    """
    Added for the multi-user rebuild (v2). A library ICT section has a small,
    known set of staff (~10-15), so this stays deliberately simple: two roles,
    email + password, no external IdP required. Swap in SSO later without
    touching the rest of the schema if the university wants it.
    """
    __tablename__ = "users"
    __table_args__ = {"extend_existing": True}
    ...
    full_name     = Column(String(256),  default="", nullable=False)
    role          = Column(String(16),   default="staff", nullable=False)  # "admin" | "staff"
    is_active     = Column(Integer,      default=1,  nullable=False)  # SQLite-friendly bool
    created_at    = Column(DateTime,     default=datetime.utcnow, nullable=False)
