...
class Repository:
    ...
    def set_document_checksum(self, doc_id: str, checksum: str) -> bool:
        """Sets checksum on a freshly-uploaded document (pdf_service.save_upload
        doesn't compute one — the caller does, so it can dedup BEFORE writing
        the file to disk). Kept separate from update_document(), which takes
        a full ProcessedDocument and does far more than this single-field set."""
        with self.session() as session:
            stmt = select(Document).where(Document.doc_id == doc_id)
    ...
    def get_document_by_checksum(self, checksum: str) -> Document | None:
        """Checksum-based duplicate detection — used by the upload endpoint
        to short-circuit re-processing an already-ingested PDF."""
        if not checksum:
            return None
        with self.session() as session:
