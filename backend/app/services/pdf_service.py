...
class PDFService:
    ...
    def save_upload(
        ...
        try:
            ...
            slog.info(
                f"Saved '{filename}' → {dest_path} ({file_size:,} bytes)"
            )
        ...
        slog.info(
            f"Document record created — status={doc.status.value}"
        )
    ...
    def load_document(
        ...
        if doc:
            logger.debug(
                f"[{doc_id}] Loaded document from DB — "
        ...
        if not state_path.exists():
            logger.warning(
                f"[{doc_id}] State file not found: {state_path}"
            )
            return None
        ...
        try:
            ...
            logger.debug(
                f"[{doc_id}] Loaded document from JSON state — "
                f"status={doc.status.value}"
            )
            ...
            logger.error(
                f"[{doc_id}] Failed to load state: {e}",
                exc_info=True,
            )
            return None
    ...
    def delete_document(
        ...
        if self.repository.delete_document(doc_id):
            slog.info(
                "Deleted document record from database"
            )
    ...
    def document_exists(
        ...
        or legacy state file.
        """

        return (
            self.repository.get_document_by_doc_id(doc_id) is not None
            or self._state_path(doc_id).exists()
        )
    ...
    def _save_document_state(
        ...
        try:
            ...
            logger.error(
                f"[{doc.doc_id}] Failed to save state: {e}",
                exc_info=True,
            )
...
def _extract_text_from_file(
    ...
    Supports:
    .docx, .doc, .txt, .xlsx, .xls, .csv, .pptx
    ...
    if suffix == ".csv":
        ...
        # Build readable text:
        # header as labels, rows as key:value lines.
        header = rows[0]
    ...
    if suffix == ".doc":
        ...
        # Otherwise fall back to raw printable text extraction.
        try:
            from docx import Document as _DocxDoc
            ...
            # Raw fallback:
            # extract printable ASCII-ish text from binary content.
            raw = file_bytes.decode(
                "latin-1",
                errors="replace",
            )
    ...
    if suffix == ".pptx":
        try:
            ...
            # heuristic picks up the deck's actual title instead of a slide
            # marker or the first bullet point. Prefer explicit document
            # properties; fall back to the first slide's title placeholder.
            deck_title = (
            ...
            if not deck_title and prs.slides:
                ...
                if (
                    first_slide.shapes.title
                    and first_slide.shapes.title.text.strip()
                ):
                    deck_title = (
                        first_slide.shapes.title.text.strip()
                    )
            ...
            for i, slide in enumerate(
                ...
                # Speaker notes can contain important
    ...
    raise RuntimeError(
        f"Text extraction is not implemented for '{suffix}' "
        f"({filename})."
    )
