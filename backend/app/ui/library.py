...
def _render_catalog() -> None:
    ...
    if not docs:
        st.info(
            "No documents yet. Upload PDFs to build the research library."
        )
        return
...
def _render_sync() -> None:
    """Removed in the v2 rebuild — Google Drive sync no longer exists.
    Upload is now handled by the FastAPI StorageService (local disk / R2)
    via the Next.js frontend, or the Batch Upload tab in this legacy UI.
    """
    st.info("Drive sync has been removed. Use Batch Upload instead, or the new web app.")
